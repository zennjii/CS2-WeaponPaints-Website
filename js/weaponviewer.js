    ...
    if(modelpath) {
        ...
        (async () => {
            try {
                ...
                console.error('Failed to load 3D model or textures:', error);
                ToggleLoading(true);
                // Show fallback image if 3D model fails
...
async function LoadTexture(texturepath, texturepath_metal) {
    ...
    try {
        ...
        console.error('Error checking texture file:', texturepath, error);
        return false;
    }
    ...
    if(texturepath_metal) {        
        try {
            ...
            if(pathexists.ok) {
                texture_metal = new THREE.TextureLoader().load(
                    texturepath_metal,
                    function(texture) {
                        console.log('Metal texture loaded successfully:', texturepath_metal);
                    },
                    function(progress) {
                        console.log('Metal texture loading progress:', (progress.loaded / progress.total * 100) + '%');
                    },
                    function(error) {
                        console.error('Failed to load metal texture:', texturepath_metal, error);
                    }
                );
                ...
                console.warn('Metal texture file not found:', texturepath_metal);
            }
        }catch(error) {
            console.error('Error checking metal texture file:', texturepath_metal, error);
        }
    }
