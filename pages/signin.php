...
<body <?= $bodyStyle ?? "" ?>>
    ...
    <form action="<?= GetPrefix(); ?>authorize" method="POST" onsubmit="document.getElementById('loading').setAttribute('data-loading', true);">
        <h2><?= $translations->login->header; ?></h2>
        <h1><a title="<?= $translations->login->titles[$title_num ?? 0]->hover; ?>" href="<?= $translations->login->titles[$title_num ?? 0]->link; ?>"><?= $translations->login->titles[$title_num ?? 0]->name; ?></a></h1>
